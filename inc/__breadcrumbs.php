...
class Theme_Breadcrumbs
{
    ...
    {
        $aria_attr = $aria_hidden ? ' aria-hidden="true"' : ' aria-label="' . esc_attr__('Home', 'islamichelp') . '"';
        $title_tag = $aria_hidden ? '' : '<title>' . esc_html__('Home', 'islamichelp') . '</title>';
    ...
    private static function get_items()
    {
        ...
        if (is_search()) {
            $items[] = array(
                'title'      => sprintf(__('Search Results for: %s', 'islamichelp'), get_search_query()),
                'url'        => '',
                'is_current' => true,
            );
            return $items;
        ...
        if (is_404()) {
            $items[] = array(
                'title'      => __('Page Not Found', 'islamichelp'),
                'url'        => '',
                'is_current' => true,
            );
            return $items;
    ...
    private static function get_post_items()
    {
        ...
        // Add archive link for other custom post types
        if ($post_type !== 'post') {
    ...
    private static function get_archive_items()
    {
        ...
        if (is_post_type_archive()) {
            ...
            $items[] = array(
                'title'      => sprintf(__('Author: %s', 'islamichelp'), $author->display_name),
    ...
    public static function render($atts = array())
    {
        ...
        ob_start();
        ?>
        <nav class="breadcrumbs-block" aria-label="<?php esc_attr_e('Breadcrumb', 'islamichelp'); ?>">
                    ...
                    <li class="breadcrumbs-block__item <?php echo $is_first ? 'breadcrumbs-block__item--first' : ''; ?> <?php echo $item['is_current'] ? 'breadcrumbs-block__item--current' : ''; ?>"
                        ...
                        <?php elseif ($item['is_current'] && $is_home) : ?>
                            <a href="<?php echo esc_url($item['url']); ?>"
                               ...
                               itemprop="item"
                               aria-label="<?php esc_attr_e('Home', 'islamichelp'); ?>">
                                <span itemprop="name" class="screen-reader-text"><?php esc_html_e('Home', 'islamichelp'); ?></span>
